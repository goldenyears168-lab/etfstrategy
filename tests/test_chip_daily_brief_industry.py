...
class IndRankTest(unittest.TestCase):
    def test_large_industry_ranks_within_group(self):
        df = pd.DataFrame({"x": list(range(1, 41)), "ind": ["A"] * 40})
        self.assertEqual(B._ind_rank(df, "x").tolist(), [i / 40 for i in range(1, 41)])

    def test_small_industry_falls_back_to_market(self):
        # B 只有 2 檔（< MIN_IND_N=3）→ 必須用全市場百分位，不是組內的 0.5/1.0
        df = pd.DataFrame({"x": list(range(1, 43)), "ind": ["A"] * 40 + ["B"] * 2})
        r = B._ind_rank(df, "x")
        self.assertAlmostEqual(r.iloc[40], 41 / 42)
        self.assertAlmostEqual(r.iloc[41], 42 / 42)

    def test_singleton_industry_is_not_pinned_to_one(self):
        # 沒有退回的話這檔會是 1.0（永遠榜首）
        df = pd.DataFrame({"x": list(range(1, 42)), "ind": ["A"] * 40 + ["Z"]})
        self.assertAlmostEqual(B._ind_rank(df, "x").iloc[40], 1.0)  # 恰為全市場最大值，合理
        df2 = df.assign(x=[41] + list(range(2, 41)) + [1])
        self.assertAlmostEqual(B._ind_rank(df2, "x").iloc[40], 1 / 41)  # 全市場最小 → 不再是 1.0
    ...
    def test_single_industry_universe_falls_back(self):
        df = pd.DataFrame({"x": [3, 1, 2], "ind": ["A"] * 3})  # 3 < MIN_IND_N
        self.assertEqual(B._ind_rank(df, "x").tolist(), [1.0, 1 / 3, 2 / 3])
...
class WeightTest(unittest.TestCase):
    def test_weight_is_equal_split(self):
        ...
        self.assertEqual(B.MIN_IND_N, 40)
