...
## Phase-5 全宇宙 + 產品化 + 收官(2026-07-31)

Phase-4 把 rev_yoy_3m 定調為「存活——真 alpha(有保留)」,但保留裡有兩件事只能靠更寬的宇宙與更多營收形態才能裁決:①90 檔大型 tilt 宇宙下的 headline(+2.34)能不能推廣到全宇宙?②L1 營收層到底只有 rev_yoy 一支腳,還是有獨立第二支?Phase-5 三條線(全宇宙裁決、L1 第二支腳探勘、月選股產品化)加一輪對抗式收官審查,把專案收斂到一張定稿真增益總帳。**結論:rev_yoy 家族比 Phase-4 想的更寬(找到兩支獨立增量腳),但 headline 績效比 Phase-4 想的更弱(全宇宙 OOS DSR 掉到 0.61 不再過關)——兩件事同時為真,方向正好互相修正。**

### 1. ★ rev_yoy_3m 全宇宙裁決:90 檔結論是否維持?

**裁決 = 維持但弱化(MAINTAINED-BUT-WEAKENED)。** 宇宙由 90 擴到 139 檔流動股(TEJ 每日 1000 配額耗盡,實測 top-139 非目標 top-300,161 檔待次日配額):

| 指標 | 原 headline(90檔) | 全宇宙(139檔) | 裁決 |
|---|---|---|---|
| IC_IS | +0.029 | **+0.028** | 穩定,真訊號 |
| IC_OOS | +0.081 | +0.034 | headline 塌陷 |
| Sharpe_IS | +1.91 | +1.91 | 維持 |
| Sharpe_OOS | +2.34 | **+1.25** | 大幅弱化 |
| perm_p | 0.002 | 0.048 | 仍過 0.05 邊緣 |
| DSR_OOS | 0.954 | **0.606** | **不再過 0.95** |
| 空頭 IC | — | +1.05 | 兩 regime 皆正 |

- **決定性對照(edge 集中在哪)**:74 檔「從未被測過的全新股」OOS IC = **+0.009(近零)**,而與原宇宙重疊的 65 檔 OOS IC = **+0.059**。→ headline 的 OOS 威力幾乎全來自原始那批股,新名字身上近乎無效。這直接量化了 Phase-4「宇宙集中」保留。
- **兩個原主張被資料翻修**:①「小型引擎」在全宇宙**翻轉**——小型 OOS +0.11、但**中型最穩定**(原報告說小型是引擎,全宇宙說中型才是骨幹)。②「產業 tilt 依賴」被**確認且更強**——產業中性化後 perm_p 0.158 完全不顯著(原本 within-sector 還殘存 0.032,全宇宙連這點都沒了)。
- **維持的部分**:IC_IS 穩定 +0.028、decile 單調(OOS rank-corr +0.85)、兩 regime 皆正。→ **rev_yoy_3m 仍是「真的橫斷面 IS 訊號」,但已不是「過 DSR 的 OOS 級 alpha」;它是一個可信的選股傾斜因子,不是可獨立部署的 alpha 引擎。**
- **可交易性/流動性保留(對抗審查補強)**:成本壓測上限僅 15bps/side,**低於台股賣方硬成本(證交稅 30bps)**;衝擊/spread/空腿 SBL 借券約束零建模;edge 落在相對小型段,真 megacap 近零。「+2.34 可否實盤交易」至今**未被回答**,不應以 headline 當可部署憑證。

### 2. L1 營收層廣度:只有 rev_yoy 一支腳,還是有獨立第二支?

**結論 = L1 不是只有一支腳。** 在最寬宇宙(418 檔 ≈ 前 420 流動、2021→2026、1351 日、annd-PIT、FinMind 還原價補 TEJ 配額缺口)測三種與 YoY 不同的營收形態,各做共線 + 殘差 IC 檢定:

| 形態 | Sh_OOS | perm_p | DSR | 與 rev_yoy_3m 橫斷共線 | 扣 rev_yoy_3m 殘差 Sh_OOS | 裁決 |
|---|---|---|---|---|---|---|
| **rev_mom**(環比 MoM 動能) | +3.19 | 0.002 | 0.999 | **+0.05(近正交)** | +2.92(≈原始) | ★ **獨立第二支腳** |
| **rev_surprise**(對 12 月對數趨勢外推超預期) | +2.59 | 0.002 | 0.899 | **−0.07(正交)** | +2.94(流動性子集殘差 +1.71>原始 +1.56) | ★ **獨立第三支腳** |
| streak(連續正成長月數) | 不穩(Sh_IS −0.65) | — | 0.117 未過 | **+0.72(高共線)** | IS 殘差塌到 +0.004 | ✗ rev_yoy 重述,非獨立 |

- **兩支近正交(|橫斷共線|≤0.07)、殘差檢定後仍保留幾乎全部預測力的獨立增量**:rev_mom(環比動能)與 rev_surprise(超預期)。streak 被證偽為 rev_yoy 的高共線重述。
- **同一弱化紀律**:原始高 Sharpe(mom +3.19)在 top-150 流動性子集**腰斬到 +1.57**,與 rev_yoy_3m 同屬中小型 tilt;仍正且顯著,但別把 +3 級數字當可部署。→ **L1 至少為「YoY 動能 + 環比動能 + 超預期」三形態、二獨立增量**,不是單腳。保留:MoM 含產業季節性(需季節調整硬化)、未做產業中性、OOS 僅 ~8 月薄窗、DSR 研究內 n=8 未跨專案折減。

### 3. 月選股產品現況(rev_yoy_monthly_screen)

已將**已驗證存活的 rev_yoy_3m 產品化成機械式月選股工具**(`scripts/research/dashboard/rev_yoy_monthly_screen.py`,argparse `--asof/--decile/--csv`),與 Phase-4 回測共用同一份 PIT 乾淨資料與同一訊號定義(名單與已驗證因子同源):

- **實跑 as-of 2026-07-31**(最新公告 2026-07-15 = 2026-06 月營收),90 檔宇宙,winsorize [−90,+300]%,多空各 decile。
  - **多方 top-decile**(高營收動能):2408 南亞科、2887 台新新光金、8299 群聯(+278.9%)、2885 元大金、2891 中信金、2344 華邦電、2337 旺宏、2059 川湖、3036 文曄、3443 創意。
  - **空方 bottom-decile**(衰退):3661 世芯-KY(−16.8%)、1519 華城、6488 環球晶、6269 台郡、5876 上海商銀、5871 中租-KY、2454 聯發科、2409 友達、1216 統一。
- **內建保留照搬未打折**:多方 10 檔有 5 檔半導體(印證約半數產業 tilt);前段金控最新單月與 3 月均背離(金融股訊噪比最低);多空全截面(非只做多綁 risk-on,符合 Phase-4「別降級成 beta」紀律);PIT 乾淨(僅取 annd≤asof)。
- **定位**:這是「把可信選股傾斜因子產品化」的工具,**不是可獨立部署的 alpha 引擎**(全宇宙 DSR 0.61)——實務每月 10-15 日跑一次,當觀盤傾斜清單用,非機械下單信號。工具本身 = ✅ 完成且實跑成功;因子等級 = 選股傾斜非 alpha 引擎。

### 4. ★ 最終「真增益總帳」(全專案收斂,一張定稿表)

經 Phase 1-5 全部證偽仍站得住的成分,連同硬保留、落層、可部署狀態:

| 成分 | 角色 | 層 | DSR / 顯著性 | 可部署狀態 | 硬保留(不可略) |
|---|---|---|---|---|---|
...
| ~~L0×L1 只做多組合~~ | ~~擇時×選股疊加~~ | — | **不採用**:選股增量 perm p=0.539 不顯著 | — | 只做多綁 risk-on 使 rev_yoy 降級成 beta(corr_champ +0.47);組合 Sharpe 2.36<L0 alone 2.58,非 1+1>2 |
| ~~融資維持率 veto~~ | ~~風控~~ | — | **不採用**:全樣本 0 天生效冗餘 | — | 作用域與 tech gate 不相交;用法是逆勢抄底非順勢 veto |
| ~~ROE / margin QoQ / EPS 動能 / streak / pb / composite~~ | ~~各式因子~~ | — | **全 null 或重述**(DSR≤0.51) | — | margin QoQ=季節噪音;EPS/streak=rev_yoy 重述;ROE-QoQ 全樣本 0.514 且 bull-only |

**一句話總帳:專案真正的淨新增 = ①一支可部署的大盤擇時系統(tech-gated champion,L0)+ ②一組「可信但非 alpha 級」的橫斷面營收選股腳(rev_yoy_3m 已產品化為月選股傾斜工具,rev_mom/rev_surprise 為研究級第二三支,L1)。** 唯一過完整 DSR 且可部署的是 L0 擇時;L1 三支腳是真的橫斷面 IS 訊號但沒有一支在全宇宙 / 可交易成本下過 OOS DSR——它們是選股傾斜,不是獨立 alpha 引擎。

### 5. 誠實收官:達到邊際遞減了嗎?

**是,已達邊際遞減,研究可收官。** 三個獨立訊號:①補實線(市場層 / 總量層 / 16 維儀表板)在 Phase 1-5 累計全數乾淨證偽,淨新增獨立 alpha = 0,champion 仍是唯一大盤領先擇時;②深化線最後的真收穫(rev_yoy 家族)在全宇宙一擴就從「過 DSR 的 alpha」弱化為「不過 DSR 的選股傾斜」——最有希望的第二支腳被更寬的資料修剪,是典型的「資料一多、overfit 就現形」收斂訊號(與 980T / adopted-44 同型);③對抗式收官審查發現的四個真風險(見 `critic_openissues.md`)全屬「措辭下修 + 成本壓測」而非「新方向」。

**最大殘餘風險(全專案共同、無法用更多回測消除)= OOS 只有單一偏多頭週期。** L0 gated champion 的 OOS 窗(2024-02+)恰好完整涵蓋 champion 史上前三強年並結構性排除唯一虧損年 2022(單獨實虧 −1.47),DSR 0.995 有「OOS 窗僥倖」成分,實盤預期應以**全樣本 Sharpe +1.80**為錨,非 OOS headline +2.68;rev_yoy 家族的 OOS 也全落在同一段多頭。**L0 與 L1 還共病於 2022**——rev_yoy 逐年 IC 唯一負年就是 2022,兩層在最需保護的趨勢空頭同時失效,不互補。live track record = 0 天,系統當前空手,「可部署」是規格意義非實盤驗證意義。

**下一步的真取捨(值得再投入 vs 到此為止):**
- **唯一真正值得做的一件**:rev_yoy_3m / rev_mom 流動性前 ~400 檔全宇宙 × **含 30bps 真稅成本** × 多空全截面市場中性形態重跑——這是唯一能把「選股傾斜」升 / 降級為「可 / 不可交易」的決定性測試,且直接回答至今未答的可交易性問題。
- **時間依賴、不宜再投入運算**:等一次完整空頭循環實測(唯一能消除單一多頭週期風險的東西,但只能等不能算);restatement vintage(無資料);更長空頭樣本;survivorship 完整合併宇宙每週再平衡確定性數字。
- **邊際遞減、到此為止**:其餘儀表板維度補實、更多營收形態變體、composite 合成(已證稀釋)。

### Phase-5 一句話總結

> **全宇宙一擴,rev_yoy_3m 從「過 DSR 的 OOS alpha」弱化為「維持但弱化的選股傾斜」(全宇宙 DSR 0.61 不過關、新股 OOS IC 近零、edge 幾乎全在原宇宙且半數產業 tilt);但同一輪把 L1 營收層從「一支腳」擴成「三形態、二獨立增量」(rev_mom 環比 + rev_surprise 超預期近正交、殘差後仍顯著,streak 證偽為重述)。rev_yoy_3m 已產品化為機械式月選股傾斜工具(實跑 as-of 2026-07-31,多空全截面、保留照搬)。專案真增益總帳定稿:唯一過 DSR 且可部署的是 tech-gated champion(L0 擇時),L1 三支營收腳是真橫斷面 IS 訊號但無一在全宇宙 / 真成本下過 OOS DSR——是選股傾斜非獨立 alpha 引擎。研究已達邊際遞減、收官;最大殘餘風險是全專案 OOS 共困於單一偏多頭週期(且 L0/L1 共病 2022 空頭),唯一值得再投入的是 rev 家族全 ~400 檔 × 含 30bps 真稅 × 市場中性形態的可交易性決定性重跑,其餘皆時間依賴或邊際遞減。**
