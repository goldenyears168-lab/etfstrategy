...
> **⚠️ 2026-07-31 更正**：`load_breadth()` 的 `%>MA` 分母原本把「當日 MA 尚未成形（歷史不足 50/200 日）或價格 NaN」的個股也算進去（`w > ma50` 對 NaN 回傳 `False` 而非 `NaN`），使 `pct_above_ma50/200` 被系統性壓低（`pct_ma50` 上限僅 ~0.32、`pct_ma200` 從不越過 50%）。已修正為 `(w > ma).astype(float).where(ma.notna() & w.notna())`，讓分母只計「當日確有有效 MA」的個股。修正後 `pct_above_ma50` 範圍 **0.01–0.96、中位 0.49**、`pct_above_ma200` 中位 **0.545**（正確跨越 50%）。下表為**修正後**實跑數字。**此 bug 只影響本廣度研究摘要，不影響任何已部署系統**（修正後定性結論不變，甚至更乾淨——見下）。
...
|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| pct_ma50_z60 | +1 | +0.022 | **+0.68** | +0.33 | +0.016 | **+0.006** | +1.03 | **−2.02** | 0.161 | 0.48 |
| mcclellan | +1 | +0.046 | +0.43 | +0.38 | +0.020 | +0.001 | +0.72 | −2.07 | 0.083 | 0.51 |
| pct_ma200_lvl | −1 | −0.020 | +0.26 | +0.65 | −0.011 | −0.039 | +0.82 | −1.74 | 0.050 | 0.38 |
| adr_z60 | +1 | +0.011 | +0.22 | +0.38 | +0.020 | −0.015 | +0.76 | −2.16 | 0.044 | 0.36 |
| net_breadth_z60 | +1 | +0.062 | +0.13 | +0.36 | +0.024 | +0.011 | +0.21 | −0.55 | 0.033 | 0.47 |
| nhnl_z60 | +1 | +0.035 | +0.00 | +0.32 | +0.020 | +0.013 | +0.17 | −1.24 | 0.021 | 0.47 |
| pct_ma200_z60 | −1 | −0.012 | −0.19 | +0.54 | +0.010 | −0.004 | +0.02 | −1.04 | 0.010 | 0.40 |
| adl_z60 | +1 | +0.027 | −0.33 | +0.25 | −0.004 | −0.013 | +0.21 | −1.84 | 0.006 | 0.24 |

**最佳存活者 `pct_ma50_z60`**：OOS Sharpe +0.68（permutation 版 +0.76），permutation **p=0.225（不顯著）**，**deflated_p=0.161（未過 0.95）**。
**冠軍 + pct_ma50_z60 50/50 組合**：OOS Sharpe **+1.13**（冠軍單獨 +1.11）——**幾乎零加分**。

> **修正前後對照（材料性變化）**：修正前的「最佳存活者」是 `pct_ma200_z60`（dir +1、OOS **+1.24**、permutation **p=0.027**、組合 **+1.47** 表面加分）。修正後 `pct_ma200_z60` **方向翻為 −1、OOS 塌為 −0.19**，跌到表尾；最佳者換成 `pct_ma50_z60`（+0.68）但 **permutation p 由 0.027 → 0.225（顯著性消失）**，冠軍組合的「表面加分」也由 +1.47 → **+1.13（等於沒加）**。另一具體印記：`pct_ma200_lvl`（=pct−0.5，>50% L0 燈）修正前因 pct 被壓在 ~0.32、恆為負，配 dir −1 後**恆做多（exposure=1.00）**——燈其實壞掉一直亮；修正後 pct 正確跨 50%，exposure 降為 0.38。**方向：定性結論不但不變，反而更乾淨**——原本要解釋的 p=0.027 表面顯著與 +1.47 表面加分都消失了。
...
1. **線性資訊量≈0**：所有訊號 `raw_IC_full` 都在 +0.02 附近，正交化掉價格動能後 `resid_IC_ctrl_mom` 更趨近 0（絕對值最高僅 +0.013，且非最佳 OOS 者）。Sharpe 幾乎全來自「二元方向閘門的曝險」，不是預測內容。
2. **edge = 多頭 regime 的做多曝險**：每個訊號 **OOS 多頭 Sharpe 強正、空頭 Sharpe 強負**（pct_ma50：+1.03 / −2.02）。這正是「同步 regime 燈」的指紋——它只是在多頭時保持做多。
3. **多重測試不過關**：修正後最佳存活者 permutation **p=0.225（已不顯著）**，全體 deflated_p ≤ 0.161 遠低於 0.95——掃了 8 個變體後選最高者本就會有這種表面波動（呼應 980T / adopted-44 過擬合史）。（修正前 pct_ma200_z60 曾有 p=0.027 的表面顯著，正是分母 bug 灌出來的假象，修正後消失。）
4. **與冠軍高度同向**：`corr_vs_champ` 0.25–0.65，冠軍組合 +1.13 相對冠軍單獨 +1.11 **等於沒加分**，證實廣度與冠軍多來自同一批多頭日的做多，非獨立分散。
...
- **與冠軍搭配（確認／過濾／前兆三格）**：
  - **確認（主用）**：冠軍給「該不該站多方」，`pct_above_ma200>50%` 給「多方是否有橫斷面支撐」。**兩者同綠才是高信賴多頭**；冠軍綠但廣度 <50%（少數權值撐盤）= 降級/減碼提示。這是廣度**唯一穩健**的用法，屬 regime 一致性檢查、不新增部位方向。（⚠️ 接入前務必用**修正後**的 `pct_above_ma200`：修正前分母 bug 把它壓在 ~0.32、**從不越過 50%**，>50% 燈等於一直不亮；修正後中位 0.545 才能正確跨門檻，此建議方屬可用。）
...
| **%>MA 分母灌水**（2026-07-31 修正） | `w > ma` 對「MA 未成形/價格 NaN」的個股回傳 `False` 而非 `NaN`，被誤計入分母 → 系統性壓低 `pct_above_ma`。改為 `(w > ma).astype(float).where(ma.notna() & w.notna())`，分母只計當日有效 MA 的個股 |
