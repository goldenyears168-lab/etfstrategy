#!/usr/bin/env python3
"""Backfill 富邦-新店 per-stock net buy/sell into stock_broker_branch_daily.

Same pipeline as backfill_yuanta_songjiang_tape.py (FinMind
TaiwanStockTradingDailyReport by securities_trader_id).

  PYTHONPATH=src .venv/bin/python scripts/research/backfill_fubon_xindian_tape.py
  PYTHONPATH=src .venv/bin/python scripts/research/backfill_fubon_xindian_tape.py \\
...
# Reuse aggregation / calendar helpers from the Yuanta backfill.
...
NAME_HINTS = ("富邦-新店", "富邦新店")
DEFAULT_TRADER_ID = DEFAULT_TRADER_ID_FUBON_XINDIAN
BRANCH_LABEL = "富邦-新店"
...
def resolve_fubon_xindian_trader(
    *,
    ...
    for r in rows:
        ...
        if any(h in name for h in NAME_HINTS) or (
            "富邦" in name and "新店" in name
        ):
            matches.append((tid, name))
    if not matches:
        raise RuntimeError(
            "TaiwanSecuritiesTraderInfo: no trader matching 富邦+新店"
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Backfill 富邦-新店 branch tape")
    ...
    trader_id, trader_name = resolve_fubon_xindian_trader(
    ...
    try:
        ...
        for i, day in enumerate(todo, 1):
            try:
                ...
                # Override fallback name if FinMind omits securities_trader.
                for row in rows:
