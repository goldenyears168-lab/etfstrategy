#!/usr/bin/env python3
"""RESEARCH FORK of ``src/tmf_channel/causal_engine.py`` — oracle-free
"sticky" segment-key entry gate (2026-08-08, Revalidate phase).
...
otherwise byte-identical ``simulate()``:
  ...
  A causal, oracle-free per-bar "sticky coarse channel key" is computed once
  per bar from ``C`` alone (same 8up/8dn slope-bucket classifier the
  upstream engine already uses at entry time for ``entry_cid_blocklist``,
  here evaluated on every bar, not just enter-bars) with min-dwell +
  k-confirm hysteresis (ported from
  ``reports/research/channel_lab/slow_cell_sticky_chop_probe_0808.py``'s
  ``sticky_keys_from_bar_cids`` / ``chop_meta_series``, verbatim hysteresis
  logic). When ``p["sticky_gate_mode"]`` is set (``"sticky_no_chop"`` or
  ``"sticky_and_pv"``), brand-new entries (``how_final == "enter"`` only —
  scale-ins/flips of an existing lot are untouched) are additionally
  rejected unless the sticky gate allows it, with an optional cancel-style
  hysteresis (``p["sticky_hyst_bars"]``) that keeps a recently-true gate
  open for N more bars. Default ``sticky_gate_mode=None`` = byte-identical
  to upstream (verified against the frozen live engine on sample days before
  this fork's walk-forward driver was trusted — see
  ``scripts/research/tx_channel_sticky_gate_walkforward.py``).

  Substitution vs. the prior probe/sweep phase: chop density there came from
  ``r_c_exante_0805.build_exante_segments``' ZigZag switch-bar list (a
  single-session research tool never validated across the 4 sanctioned
  multi-week windows). Here "switches" are simply raw-cid8 transition bars
  from this engine's own per-bar cid8 stream — still purely causal, still
  oracle-free, but self-contained inside the engine fork instead of
  depending on a separate single-day-validated module.

Original upstream docstring follows unmodified for context/provenance.

---

TMF channel causal engine (Order + research SSOT).
...
# resolution accordingly (sibling "src" is a cousin, not the parent, here).
...
# --- STICKY GATE FORK: oracle-free per-bar coarse channel key + hysteresis
# (ported from reports/research/channel_lab/slow_cell_sticky_chop_probe_0808.py
# sticky_keys_from_bar_cids / chop_meta_series, verbatim hysteresis logic).
_STICKY_UPS = (3.0, 5.0, 7.0, 10.0, 13.0, 16.0, 20.0, 28.0)
_STICKY_COARSE_UP = (("Us1", (1, 2)), ("Us2", (3, 4)), ("Us3", (5, 6)), ("Us4", (7, 8)))
_STICKY_COARSE_DN = (("Ds1", (1, 2)), ("Ds2", (3, 4)), ("Ds3", (5, 6)), ("Ds4", (7, 8)))


def _sticky_cid8(C, t: int, look: int = 20) -> str:
    """Same slope-bucket classifier already used inline for entry_cid_blocklist
    (causal_engine.py L1471-1485), evaluated at an arbitrary bar t instead of
    only at enter-bars."""
    lk = min(look, t)
    if lk >= 1:
        m8 = float((C[t] - C[t - lk]) / lk)
    else:
        m8 = 0.0
    ups = _STICKY_UPS
    dns = [-x for x in ups]
...
def _sticky_cid_to_coarse(cid: str) -> str:
    if not cid or cid[0] not in "UD" or len(cid) < 2:
        return "na"
    side, n = cid[0], int(cid[1:])
    table = _STICKY_COARSE_UP if side == "U" else _STICKY_COARSE_DN
...
def sticky_keys_causal(C, *, min_dwell: int = 8, k_confirm: int = 2, look: int = 20) -> list[str]:
    """Per-bar sticky coarse key with hysteresis (causal: bar t only uses
    C[0..t]). Ports sticky_keys_from_bar_cids()'s hysteresis verbatim, fed
    from this engine's own cid8 stream instead of r_c_exante_0805 segments."""
    n = len(C)
    out = ["na"] * n
    if n == 0:
        return out
    cur = _sticky_cid_to_coarse(_sticky_cid8(C, 0, look))
    dwell = 0
    pending: str | None = None
    pend_n = 0
    for i in range(n):
        want = _sticky_cid_to_coarse(_sticky_cid8(C, i, look))
        dwell += 1
        if want == cur:
            ...
            if pending == want:
                ...
                pending, pend_n = want, 1
            if dwell >= min_dwell and pend_n >= k_confirm:
                cur = want
                dwell = 0
                pending, pend_n = None, 0
        out[i] = cur
...
def sticky_chop_causal(
    C, *, look_cid: int = 20, chop_look: int = 30, chop_thr: int = 3, chop_min_dwell: int = 10
) -> list[str]:
    """Rolling raw-cid8-transition density → sticky chop|dir overlay (ports
    chop_meta_series() verbatim; "switches" = this engine's own raw cid8
    transition bars, not r_c_exante_0805 ZigZag confirms)."""
    n = len(C)
    if n == 0:
        return []
    raw_cid = [_sticky_cid8(C, i, look_cid) for i in range(n)]
    sw = {i for i in range(1, n) if raw_cid[i] != raw_cid[i - 1]}
    raw = []
    for t in range(n):
        a = max(0, t - chop_look + 1)
        cnt = sum(1 for i in range(a, t + 1) if i in sw)
        raw.append("chop" if cnt >= chop_thr else "dir")
    out = ["dir"] * n
    cur = raw[0]
    dwell = 0
    pending, pend_n = None, 0
    for i, want in enumerate(raw):
        ...
        if want == cur:
            ...
            if pending == want:
                ...
                pending, pend_n = want, 1
            if dwell >= chop_min_dwell and pend_n >= 2:
                cur = want
                dwell = 0
                pending, pend_n = None, 0
        out[i] = cur
...
def sticky_allow_causal(
    ...
    *,
    ...
    chop_look: int = 30,
    chop_thr: int = 3,
    chop_min_dwell: int = 10,
    hyst_bars: int = 0,
) -> tuple[list[bool], list[str], list[str]]:
    """Boolean entry-allow gate. mode: sticky_no_chop | sticky_and_pv.

    sticky_no_chop: sticky key set AND not in chop.
    sticky_and_pv : sticky_no_chop AND raw PV8 not in (dry, na) (mirrors
                    probe's pv_allow_series: pv != "dry" and pv != "na").
    hyst_bars>0 applies cancel-style hysteresis: once allow goes True, it
    stays True for hyst_bars more bars after the raw condition drops.
    """
    n = len(C)
    sticky = sticky_keys_causal(C, min_dwell=min_dwell, k_confirm=k_confirm)
    chop = sticky_chop_causal(
        C, chop_look=chop_look, chop_thr=chop_thr, chop_min_dwell=chop_min_dwell
    )
    raw = []
    for i in range(n):
        ok_s = sticky[i] not in ("", "na") and chop[i] != "chop"
        if mode == "sticky_no_chop":
            raw.append(ok_s)
        elif mode == "sticky_and_pv":
            r = reg_raw[i] if i < len(reg_raw) else "na"
            raw.append(ok_s and r not in ("dry", "na"))
        else:
            raise ValueError(f"unknown sticky_gate_mode: {mode!r}")
    if hyst_bars <= 0:
        return raw, sticky, chop
    out = [False] * n
    hold = 0
...
def simulate(
    ...
    p = dict(
        ...
        # STICKY GATE FORK (research-only, not in upstream engine): None/""
        # = disabled (byte-identical to upstream). "sticky_no_chop" or
        # "sticky_and_pv" gates brand-new entries (how_final=="enter" only)
        # on the causal sticky-segment-key allow series; see
        # sticky_allow_causal() above.
        sticky_gate_mode=None,
        sticky_hyst_bars=0,
        sticky_min_dwell=8,
        sticky_k_confirm=2,
        sticky_chop_look=30,
        sticky_chop_thr=3,
        sticky_chop_min_dwell=10,
    ...
    # STICKY GATE FORK: precompute causal entry-allow gate once per bar.
    # sticky_gate_mode=None (default) => _stk_allow=None => zero behavior
    # change below (byte-identical to upstream).
    _stk_mode = p.get("sticky_gate_mode")
    _stk_allow: list[bool] | None = None
    if _stk_mode:
        _stk_reg_raw = [classify_pv(C, O, rvol, i)[0] for i in range(n)]
        _stk_allow, _stk_key_seq, _stk_chop_seq = sticky_allow_causal(
            C,
            _stk_reg_raw,
            mode=_stk_mode,
            min_dwell=int(p.get("sticky_min_dwell", 8)),
            k_confirm=int(p.get("sticky_k_confirm", 2)),
            chop_look=int(p.get("sticky_chop_look", 30)),
            chop_thr=int(p.get("sticky_chop_thr", 3)),
            chop_min_dwell=int(p.get("sticky_chop_min_dwell", 10)),
            hyst_bars=int(p.get("sticky_hyst_bars", 0)),
        )
    ...
    def open_lot(t, side, px, how, reg, rv, *, honor_limit: bool = False, hung_limit=None):
        ...
        if how_final == "enter" and bl:
            ...
            if cid8 in set(bl):
                ...
                return False
        # STICKY GATE FORK: reject brand-new entries the causal sticky-key
        # allow series vetoes. how_final=="enter" only (matches the scope of
        # every other entry-only reject above) — never gates scale-in/flip.
        if how_final == "enter" and _stk_allow is not None and not _stk_allow[t]:
            log(t, "reject", side, px, "sticky_gate")
            return False
        was_flat = not lots
        improv = 0.0
    ...
    for t in range(n):
        ...
        if t > 0 and short_px is not None and short_px <= C[t - 1]:
            log(t, "cancel", "S", short_px, f"stale_through|{reg}")
            last_short_rail = short_px
            short_px = short_born = None
            short_greed = False
        if t > 0 and long_px is not None and long_px >= C[t - 1]:
            log(t, "cancel", "L", long_px, f"stale_through|{reg}")
            last_long_rail = long_px
            long_px = long_born = None
            long_greed = False
        ...
        if sticky:
            ...
            amend_every = p["amend"] * (p["contract_amend_slow"] if reg == "contract" else 1)
        ...
        if do_place or do_amend:
            ...
            _cell = dict((_spb.get(_sess_b) or {}).get(reg) or {})
            ...
            want_s, want_l, sh, slv = birth_wants(t, reg, tilt, fade, use_lo, use_hi)
            ...
            strong_up, strong_dn = trend_dampen_flags(t, reg, tilt)
            if strong_up:
                place_s = False
                note = note + "|dampen_S"
            if strong_dn:
            ...
            if (_psb or _cell.get("block")) and not lots:
                ...
                _sides = list((_psb.get(_sess) or {}).get(reg) or [])
                ...
                if "S" in _sides:
                    place_s = False
                    if short_px is not None:
                        log(t, "cancel", "S", short_px, f"pvBlock:{_sess}|{reg}")
                        last_short_rail = short_px
                        short_px = short_born = None
                    note = note + f"|pvBlock_S:{reg}"
                if "L" in _sides:
                    place_l = False
                    if long_px is not None:
                        log(t, "cancel", "L", long_px, f"pvBlock:{_sess}|{reg}")
                        last_long_rail = long_px
                        long_px = long_born = None
                    note = note + f"|pvBlock_L:{reg}"
        ...
        if not lots:
            su_f, sd_f = trend_dampen_flags(t, reg, tilt)
        ...
        if p.get("tick_native") and tick_index is not None:
            ...
            allow_flip = p["allow_flip"] and not (reg == "contract" and p["contract_no_flip"])
            ...
            if hit_s:
                ...
                if lots and lots[0]["s"] == "L":
                    if allow_opp_cover and not block_exit:
                        close_one(t, px, "opp_cover", prefer_side="L")
                        if allow_flip and not lots:
                            open_lot(t, "S", px, "flip", reg, rv, honor_limit=True, hung_limit=hung_s)
                        short_px = short_born = None
                elif not lots:
                    if open_lot(
                        t, "S", px, "enter", reg, rv, honor_limit=True, hung_limit=hung_s
                    ):
                        short_px = short_born = None
                    # reject must NOT pull the resting limit (real exchange keeps it)
                elif lots[0]["s"] == "S":
                    if open_lot(
                        t, "S", px, "scale_in", reg, rv, honor_limit=True, hung_limit=hung_s
                    ):
                        short_px = short_born = None
                    elif len(lots) >= p["max_lots"]:
            ...
            if hit_l:
                ...
                if lots and lots[0]["s"] == "S":
                    if allow_opp_cover and not block_exit:
                        close_one(t, px, "opp_cover", prefer_side="S")
                        if allow_flip and not lots:
                            open_lot(t, "L", px, "flip", reg, rv, honor_limit=True, hung_limit=hung_l)
                        long_px = long_born = None
                elif not lots:
                    if open_lot(
                        t, "L", px, "enter", reg, rv, honor_limit=True, hung_limit=hung_l
                    ):
                        long_px = long_born = None
                elif lots[0]["s"] == "L":
                    if open_lot(
                        t, "L", px, "scale_in", reg, rv, honor_limit=True, hung_limit=hung_l
                    ):
                        long_px = long_born = None
                    elif len(lots) >= p["max_lots"]:
                        log(t, "cancel", "L", long_px, "max_lots_rail")
    ...
    return trades, events, ws, wl, rvol, regime, open_pos
...
def main():
    ...
    for name, kw in [
        ...
        tr, ev, ws, wl, rvol, regime, _op = simulate(O, H, L, C, V, T, kw, vix_delta=vix)
        s = summarize(tr)
    ...
    tr, ev, ws, wl, rvol, regime, _op = simulate(O, H, L, C, V, T, None, vix_delta=vix)
    s = summarize(tr)
