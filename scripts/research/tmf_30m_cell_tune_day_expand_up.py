#!/usr/bin/env python3
"""Cell tune (2026-08-09): day|expand_up under the 30-min-primary PV8 regime
architecture (see scripts/research/tmf_30m_primary_1m_calib_prototype.py --
NOT touched here, only imported).

Assigned cell: day|expand_up. This script tunes ONLY this cell's params
...
IMPORTANT baseline fact discovered while wiring this up: in the CURRENT
live book, day|expand_up is already block=["L","S"] (DAY_BLOCKS, never
touched by CELL_TUNE_V2_PATCHES) -- i.e. it fires ZERO trades today, on
either the 1-min or 30-min regime feed. So the "current default" baseline
for this cell contributes a fixed net-pnl-delta of 0 on every day by
construction, and the entire question is whether *unblocking* it (with
tuned hang/hold params, now that the regime it keys off persists ~30min
instead of ~1-5min) can extract a positive edge, day-clustered and paired
against that all-zero baseline.
...
CANDIDATES = [
    ...
    ("unblock_wide_band_30_45", dict(block=[], hang_lo=25.0, hang_hi=45.0)),
    ("unblock_wide_band_hold45", dict(
        block=[], hang_lo=30.0, hang_hi=55.0, max_hold_bars=45)),
    ("long_only_same_band", dict(block=["S"])),
    ("long_only_wide_band_hold45", dict(
        block=["S"], hang_lo=25.0, hang_hi=45.0, max_hold_bars=45)),
    ("long_only_wide_band_hold60", dict(
        block=["S"], hang_lo=30.0, hang_hi=55.0, max_hold_bars=60)),
    ("short_only_same_band", dict(block=["L"])),
    ("unblock_tight_eager", dict(
        block=[], hang_lo=20.0, hang_hi=38.0, max_hold_bars=20,
        early_fill_gamma=12.0)),
]
...
def paired_stats(deltas: list[float]):
    ...
    if sd == 0:
        ...
        t = mean / (sd / (n ** 0.5))
    # crude two-sided p from t via normal approx (n is small so this is a
    # rough guide, reported alongside n -- not a substitute for judgment)
...
def main():
    ...
    per_cand_delta, per_cand_n, baseline_n = evaluate(IN_SAMPLE_DAYS, recipe_base, vix)
    print(f"baseline day|expand_up trade counts (should be all 0): "
          f"{sorted(set(baseline_n.values()))}")
    ...
    if thin:
        ...
        # rank by mean delta among candidates with >=15 trades
    ...
    if best_label is None:
        print("\nFINAL VERDICT: NO_IMPROVEMENT (or INSUFFICIENT_DATA, see above) "
              "-- keep current default (block=['L','S']) for day|expand_up")
        return
    ...
    for d in oos_days:
        ...
        if arr is None:
            continue
        bnet, _ = run_book(d, arr, baseline_book, recipe_base, vix)
    ...
    print("\n=== FINAL ===")
    print(f"cell=day|expand_up candidate={best_label} overrides={best_overrides}")
