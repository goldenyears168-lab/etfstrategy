#!/usr/bin/env python3
"""H1-WHALE-BUY-FORWARD · forward-return event study.

讀 scan_whale_buy_concentration.py 產出的事件清單，把同一 (stock_id,
top1_trader_id) 短窗內連續事件合併成一個「行情」（只取首日進場），算事件後
N 日 excess return（vs IX0001），依 config/research_splits.yaml
default_is_oos（70/30 date_ratio）拆 IS/OOS，輸出敘述統計 + report md。
...
def branch_track_record(result: pd.DataFrame, branch_names: dict[str, str]) -> pd.DataFrame:
    """依 top1_trader_id 分組（非彙總全市場），找出哪些分點自己的集中度事件
    有正向 forward return 代表性——這是「記住哪些分點值得追蹤」的核心輸出，
    跟 H1 的全市場 pooled 假說是分開的兩件事：全市場沒 edge 不代表每個分點
    都沒有 edge，可能只是少數分點真的有資訊優勢、被其他雜訊分點稀釋掉。"""
    rows = []
...
def render_branch_section(track_record: pd.DataFrame, branch_csv_name: str) -> list[str]:
    lines = [
        ...
        "",
        f"依 Top1 分點（{'/'.join(f'{h}日' for h in HORIZONS)} horizon）分組，"
        f"用 H{PRIMARY_HORIZON} 平均 excess return 排序。"
    ...
    lines.append("")
    lines.append(
        f"已知專家分點池（9217/9801/9661/9227）中，這次有出現在集中度事件裡的："
...
def render_report(
    ...
    lines = [
        "# H1-WHALE-BUY-FORWARD · 分點買超集中度 forward return 事件研究",
        "",
        f"產出時間：{now}",
        "",
        "topic：`config/research.yaml#topics.whale-buy-concentration`",
        "",
        "## 樣本",
        "",
        f"- 金額門檻：Top1+2 ≥ {amt_floor/1e8:.1f}億",
        f"- 原始事件（單日）：{raw_n}",
    ...
    is_weak = is_h9_win is not None and is_h9_win < 55
    lines += [
        "",
    ...
    if is_weak:
        lines.append(
            f"**全市場 pooled 版本目前看不出穩健 edge**（IS H{PRIMARY_HORIZON} 勝率 "
            f"{is_h9_win}%，接近或低於隨機）。這不代表所有分點都沒有 edge——見下方"
            "「分點層級追蹤紀錄」，可能是少數分點真的有資訊優勢，被其他雜訊分點"
            "稀釋掉了。pooled 版本暫不建議往 Strategy/Order 層推進。"
        )
    else:
        lines.append(
            f"全市場 pooled 版本 IS H{PRIMARY_HORIZON} 勝率 {is_h9_win}%，"
            "有一定跡象但仍需 OOS 樣本補足才能下定論（見上方 OOS 樣本數警告）。"
        )
    lines += branch_lines
    lines += [
        "",
        "## 方法論摘要",
        "",
        "- 事件定義：`scripts/research/scan_whale_buy_concentration.py`"
        f"（Top1+2 買超占比≥80%、金額≥{amt_floor/1e8:.1f}億、Top3占比<10%、"
        "出手分點數≥15、排除 mega 黑名單與 ETF）",
        "- 行情合併：同一 (stock_id, top1_trader_id) 在 ≤10 個交易日內的連續"
        "事件視為同一波吃貨，僅取首個事件日為進場點，避免偽獨立樣本灌水統計量",
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-tag",
        default="",
        help="需與 scan_whale_buy_concentration.py 的 --out-tag 一致，讀取對應 variant 的事件檔",
    )
    args = ap.parse_args()
    ...
    report_path = OUT_DIR / f"whale_buy_concentration_study{suffix}.md"
    branch_track_record_csv = OUT_DIR / f"whale_buy_concentration_branch_track_record{suffix}.csv"
    campaigns_out = OUT_DIR / f"whale_buy_concentration_campaigns{suffix}.csv"
