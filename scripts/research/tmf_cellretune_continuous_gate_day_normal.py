#!/usr/bin/env python3
"""Retune day|normal cell under the NEW continuous (per-bar) NQ session
...
stay at the current-live default throughout; only day|normal's
...
day|normal trades minus baseline net pnl for the SAME cell on the SAME
...
def main():
    ...
    print(f"\nbaseline in-sample n_trades total for {SESS}|{PV} (continuous gate) = "
          f"{total_base_n} across {len(IN_SAMPLE_DAYS)} days")
    ...
    candidates = [
        ("tighter_8_20", dict(hang_lo=8.0, hang_hi=20.0)),
        ("current_narrow_12_27", dict(hang_lo=12.0, hang_hi=27.0)),  # sanity == baseline
        ("wider_16_32", dict(hang_lo=16.0, hang_hi=32.0)),
        ("wider_20_40", dict(hang_lo=20.0, hang_hi=40.0)),
        ("hold_shorter_20", dict(max_hold_bars=20)),
        ("hold_longer_50", dict(max_hold_bars=50)),
        ("wider_hold_longer", dict(hang_lo=16.0, hang_hi=32.0, max_hold_bars=50)),
        ("gamma_lower_6", dict(early_fill_gamma=6.0)),
        ("gamma_higher_18", dict(early_fill_gamma=18.0)),
    ...
    out_path = "/tmp/tmf_cellretune_continuous_gate_day_normal_insample.json"
    ...
    print(f"\n=== Best in-sample candidate: {best_name} mean_delta={best['stats']['mean']:.2f} "
          f"t={best['stats']['t']} p={best['stats']['p']} ===")

    if best["stats"]["mean"] <= 0 or best_name == "current_narrow_12_27":
        print("No candidate beats current default in-sample (or the winner IS the current "
              "default's near-restatement). FINAL VERDICT: NO_IMPROVEMENT (keep current default).")
        return
    ...
    out_path2 = "/tmp/tmf_cellretune_continuous_gate_day_normal_oos.json"
