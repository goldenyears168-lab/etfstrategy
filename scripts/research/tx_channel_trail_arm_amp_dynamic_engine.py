...
def simulate(
    ...
    def open_lot(t, side, px, how, reg, rv, *, honor_limit: bool = False, hung_limit=None):
        ...
        # RESEARCH FORK (trail_arm amp+volume dynamic, 2026-08-08): if a
        # precomputed causal per-bar array p["trail_arm_dyn_series"] is
        # supplied, freeze this trade's trail_arm_pts at the value observed
        # AT ENTRY (bar t) rather than the fixed scalar. Array must already
        # be causal (computed only from bars <= t) by the caller. None/absent
        # => byte-identical fixed-threshold behavior (baseline).
        _arm_dyn_series = p.get("trail_arm_dyn_series")
        _arm_dyn = None
        if _arm_dyn_series is not None and t < len(_arm_dyn_series):
            v = _arm_dyn_series[t]
            if v is not None:
                _arm_dyn = float(v)
        lots.append(
    ...
    def _tick_walk_bar(t: int) -> None:
        ...
        _arm_dyn0 = lots[0].get("arm_dyn") if lots else None
        arm = float(_arm_dyn0 if _arm_dyn0 is not None else (p.get("trail_arm_pts") or 50.0))
        ...
        if p.get("tick_native") and tick_index is not None:
            ...
            if lots and exit_mode in ("trail", "structure", "hybrid_trail"):
                ...
                if hold_b >= min_smart and exit_mode in ("trail", "hybrid_trail"):
                    _arm_dyn1 = lots[0].get("arm_dyn") if lots else None
                    arm = float(_arm_dyn1 if _arm_dyn1 is not None else (p.get("trail_arm_pts") or 50.0))
                    give = float(p.get("trail_giveback_pts") or 40.0)
                    if peak_fav >= arm and (peak_fav - fav_now) >= give:
                        while lots:
                            close_side(
