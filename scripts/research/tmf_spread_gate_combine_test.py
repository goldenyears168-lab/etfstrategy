#!/usr/bin/env python3
"""2026-08-11: combine (not replace) the existing NQ direction gate with the
TW-US 5h MA-deviation spread signal -- only allow entry when BOTH agree on
direction. Preserves the NQ gate's own already-validated contribution while
adding the new (IS p<0.001, OOS p<1e-6 IC) spread signal as a confirmation
filter, rather than tmf_spread_gate_backtest.py's full-replacement approach
(consistently positive direction but not significant, IS p>=0.107, OOS
p>=0.33 across all thresholds).

combined_gate(t):
  base = existing continuous NQ gate (L/S/none)
  spread_side = "S" if spread(t)>=threshold, "L" if spread(t)<=-threshold, else "none"
  -> base if base == spread_side else "none"
"""
...
def combined_gate_for_day(day, T, C, base_gate, bundle, min_age, threshold, us_cache):
    out = {}
    roll_sum = sum(C[:TW_WINDOW_MIN]) if len(C) >= TW_WINDOW_MIN else 0.0
    for i, t in enumerate(T):
        base = base_gate.get(t, "none")
        if base == "none":
            out[t] = "none"
            continue
        if i < TW_WINDOW_MIN:
        ...
        spread_side = "S" if spread >= threshold else ("L" if spread <= -threshold else "none")
        out[t] = base if base == spread_side else "none"
    return out
...
def run_window(label, days, source_map, recipe_base, vix, bundle, min_age, threshold):
    ...
    for d in days:
        ...
        if arr is None:
            continue
        O, H, L, C, V, T = arr
        base_gate = continuous_gate_for_day(d, T, source=_source_for(d, source_map))
        recipeB = deepcopy(recipe_base)
        recipeB["session_side_gate"] = base_gate
        tradesB, *_ = simulate(O, H, L, C, V, T, recipeB, vix_delta=vix)
        netB = sum(t["pnl"] for t in tradesB)

        cgate = combined_gate_for_day(d, T, C, base_gate, bundle, min_age, threshold, us_cache)
        recipeC = deepcopy(recipe_base)
        recipeC["session_side_gate"] = cgate
...
def main():
    ...
    print("=== IS(22d) screen across thresholds (relaxed, 2026-08-11) ===")
    is_results = {}
