#!/usr/bin/env python3
"""RESEARCH FORK (scripts/research/, not live SSOT) of tmf_channel/causal_engine.py.

Adds p["struct_loss_gated"]: when True, struct_break exits are only allowed
to fire while the open position is currently underwater (fav_now < 0) --
i.e. struct_break becomes a pure loss-mitigation overlay instead of also
cutting flat/profitable trades that the (weak) real reversion tendency
might otherwise have let run. Everything else byte-identical to the
upstream engine at the time of the fork (2026-08-08).
        ...
        for k in range(start, end):
            ...
            if lots and exit_mode in ("trail", "structure", "hybrid_trail"):
                ...
                if lots and hold_b >= min_smart and exit_mode in ("structure", "hybrid_trail"):
                    ...
                    if (
                        p.get("struct_disabled")
                        or (p.get("day_struct_disabled") and _day_s)
                        or (p.get("struct_loss_gated") and fav_now >= 0)
                    ):
                        pass
                    else:
