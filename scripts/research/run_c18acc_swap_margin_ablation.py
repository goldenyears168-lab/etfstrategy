...
用法：
  PYTHONPATH=src python3 scripts/research/run_c18acc_swap_margin_ablation.py
  PYTHONPATH=src python3 scripts/research/run_c18acc_swap_margin_ablation.py --variants M0,M1,M4
"""
...
from research.backtest.c18acc_swap_margin_ablation import (  # noqa: E402
    MARGIN_VARIANTS,
    render_swap_margin_ablation_md,
    run_swap_margin_ablation,
)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="C18acc swap margin ablation hold-out")
    ...
    variants = [v for v in MARGIN_VARIANTS if v["id"] in want]
    if not variants:
    ...
    try:
        payload = run_swap_margin_ablation(
            conn,
    ...
    out_json = args.out or RESEARCH_RRG / f"{stamp}_c18acc_swap_margin_ablation.json"
    out_md = out_json.with_suffix(".md")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    slim = {**payload}
    # drop heavy period lists if any leaked (we only store stats in variants)
    out_json.write_text(json.dumps(slim, ensure_ascii=False, indent=2), encoding="utf-8")
    out_md.write_text(render_swap_margin_ablation_md(payload), encoding="utf-8")
