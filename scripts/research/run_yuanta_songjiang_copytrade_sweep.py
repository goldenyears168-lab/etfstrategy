#!/usr/bin/env python3
"""元大-松江 Copytrade · Phase A sweep + Phase B swap overlay.

  PYTHONPATH=src .venv/bin/python scripts/research/run_yuanta_songjiang_copytrade_sweep.py
  PYTHONPATH=src .venv/bin/python scripts/research/run_yuanta_songjiang_copytrade_sweep.py \\
...
from copytrade.branch_signals import DEFAULT_TRADER_ID_YUANTA_SONGJIANG  # noqa: E402
...
def main() -> int:
    ap = argparse.ArgumentParser(description="元大-松江 copytrade parameter sweep")
    ap.add_argument("--db", type=Path, default=DEFAULT_DB_PATH)
    ap.add_argument("--start", default="2024-07-01")
    ap.add_argument("--end", default=date.today().isoformat())
    ap.add_argument("--trader-id", default=DEFAULT_TRADER_ID_YUANTA_SONGJIANG)
    ap.add_argument(
        "--quick",
        action="store_true",
    ...
    try:
        ...
        if int(n_tape) == 0:
            print(
                ...
                "Run scripts/research/backfill_yuanta_songjiang_tape.py first.",
                file=sys.stderr,
            )
            return 2
        print(
