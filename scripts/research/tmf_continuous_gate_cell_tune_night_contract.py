#!/usr/bin/env python3
"""Continuous-gate PV16 cell retune: ASSIGNED CELL = night|contract.
...
behaves differently, is the CURRENT-LIVE night|contract cell's own
...
Current-live night|contract cell (from specialized_cell_book()):
  hang_lo=16.0, hang_hi=30.0, early_fill_gamma=5.0, max_hold_bars=20,
  block=[] (both sides live), skip_quiet_mode="dry", bias=True,
  vixtwn_calib="none".

Methodology: same day-clustered paired re-simulation rigor as every prior
cell-tune script in this directory. Only night|contract is varied; all
...
def main():
    ...
    # BASE_CELL: hang_lo=16.0, hang_hi=30.0, max_hold_bars=20, gamma=5.0,
    # block=[] (unblocked both sides). Sweep hang band (tighter/wider than
    # current), hold bars, gamma, single-side blocks, and full block since
    # the continuous gate changes how often/when this regime is steered.
    candidates = {
        "current_default": None,
        "tight_12_24": dict(hang_lo=12.0, hang_hi=24.0),
        "wide_20_36": dict(hang_lo=20.0, hang_hi=36.0),
        "very_wide_24_44": dict(hang_lo=24.0, hang_hi=44.0),
        "hold12": dict(max_hold_bars=12),
        "hold30": dict(max_hold_bars=30),
        "gamma8": dict(early_fill_gamma=8.0),
        "tight_hold12": dict(hang_lo=12.0, hang_hi=24.0, max_hold_bars=12),
        "wide_hold30": dict(hang_lo=20.0, hang_hi=36.0, max_hold_bars=30),
        "longonly": dict(block=["S"]),
        "shortonly": dict(block=["L"]),
    ...
    if n_trades_total < 15:
        print(f"\n*** THIN SAMPLE: baseline only generated {n_trades_total} trades across "
              f"22 in-sample days. Any candidate's delta stats are unreliable regardless of "
              f"sign. Verdict=INSUFFICIENT_DATA. ***")
        return
    ...
    if best_stats["mean"] <= 0:
        print("\nNo candidate beats the current-live default in-sample. "
