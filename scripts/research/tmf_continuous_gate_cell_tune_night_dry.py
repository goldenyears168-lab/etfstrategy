#!/usr/bin/env python3
"""Cell-tune (2026-08-10): night|dry, retuned under the NEW continuous NQ/ES
...
ASSIGNED CELL: night|dry only. All other 15 cells stay at the current
...
every run in this script -- only night|dry's hang_lo/hang_hi/max_hold_bars/
...
so the comparison isolates night|dry's own params under the new gate regime,
...
BASE_CELL = dict(specialized_cell_book()["night"]["dry"])
# = {hang_lo: 14.0, hang_hi: 28.0, early_fill_gamma: 5.0, max_hold_bars: 20,
#    block: [], skip_quiet_mode: "dry", bias: True, vixtwn_calib: "none"}
...
CANDIDATES: dict[str, dict] = {
    ...
    # just re-confirming the old tight 14-28 band.
    "narrow": dict(hang_lo=9.0, hang_hi=20.0),
    "wide": dict(hang_lo=18.0, hang_hi=34.0),
    "wider_still": dict(hang_lo=22.0, hang_hi=40.0),
    "wide_shorthold": dict(hang_lo=18.0, hang_hi=34.0, max_hold_bars=12),
    "wide_longhold": dict(hang_lo=18.0, hang_hi=34.0, max_hold_bars=30),
    "current_shorthold": dict(max_hold_bars=12),
    "current_longhold": dict(max_hold_bars=30),
    "current_gamma_up": dict(early_fill_gamma=10.0),
...
def main():
    ...
    print(f"\n[baseline] total night|dry appearances={base_total_n}, net={base_total_net:.1f} "
          f"over {len(IN_SAMPLE_DAYS)} days")

    if base_total_n < 15:
        print("\n=== INSUFFICIENT DATA: fewer than ~15 night|dry appearances in-sample ===")
        out = dict(
            cell="night|dry", base_cell=BASE_CELL,
