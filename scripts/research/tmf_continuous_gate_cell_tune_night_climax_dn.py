#!/usr/bin/env python3
"""Continuous-gate PV16 cell retune: ASSIGNED CELL = night|climax_dn.
...
behaves differently, is the CURRENT-LIVE night|climax_dn cell's own
...
NOTE: current-live night|climax_dn is block=["L"] (longs blocked, shorts
allowed) -- not fully blocked, unlike night|climax_up. The sweep below
covers both directions: tighter/wider hang bands, alternate hold windows,
unblocking longs too (block=[]), and fully blocking both sides
(block=["L","S"]) as the conservative floor.

Methodology: same day-clustered paired re-simulation rigor as every prior
cell-tune script in this directory. Only night|climax_dn is varied; all
...
def main():
    ...
    print("NOTE: current-live cell is block=['L'] (short-only; longs blocked).")
    ...
    if thin:
        print(f"\n*** THIN SAMPLE: only {n_trades_total} baseline trades across 22 in-sample "
              f"days. Will still sweep (unblocked variants may generate more trades) but any "
              f"winning candidate that itself stays under ~15 trades gets INSUFFICIENT_DATA. ***")

    # BASE_CELL (current live): hang_lo=18.0, hang_hi=32.0, early_fill_gamma=5.0,
    # max_hold_bars=20, block=['L'] (short-only). Under the continuous gate,
    # session-side steering/blocking happens far more often mid-session than
    # under the frozen-at-open anchor, so sweep both tighter (faster exits,
    # less exposure to gate churn) and wider (fewer forced early exits) hang
    # bands, a couple of max_hold options, unblocking longs too, and fully
    # blocking both sides as the conservative floor.
    candidates = {
        "current_default": None,
        "tighter_14_26": dict(hang_lo=14.0, hang_hi=26.0),
        "tighter_14_26_hold14": dict(hang_lo=14.0, hang_hi=26.0, max_hold_bars=14),
        "wider_22_38": dict(hang_lo=22.0, hang_hi=38.0),
        "wider_22_38_hold30": dict(hang_lo=22.0, hang_hi=38.0, max_hold_bars=30),
        "current_hold12": dict(max_hold_bars=12),
        "current_hold30": dict(max_hold_bars=30),
        "current_gamma0": dict(early_fill_gamma=0),
        "current_gamma12": dict(early_fill_gamma=12.0),
        "unblock_both": dict(block=[]),
        "blocked_both": dict(block=["L", "S"]),
    }
