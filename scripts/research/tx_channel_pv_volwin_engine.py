#!/usr/bin/env python3
"""RESEARCH FORK of ``src/tmf_channel/causal_engine.py`` — PV8 classifier
VOL_WIN sweep (2026-08-08, PV8-classification-mechanism audit).

NOT the live engine. Frozen live SSOT is ``src/tmf_channel/causal_engine.py``
(never edited by this fork). This copy adds exactly ONE new knob:
``simulate()`` now reads ``p.get("pv_vol_win", VOL_WIN)`` and passes it as
``win=`` to ``rvol_series()`` (upstream hardcodes ``rvol_series(V)`` == 20).
Everything else — thresholds (DRY/CONTRACT/EXPAND/CLIMAX), the 5-bar impulse
window inside ``classify_pv``, all fill/exit logic — is byte-identical to
upstream. Default (key absent) reproduces the live engine exactly.

Motivation: the assigned audit dimension is the PV8 classifier's own
architecture (rvol = V[t]/median(V[t-19:t+1]), VOL_WIN=20). Prior research
(``regime_threshold_wide_sensitivity.json``) only perturbed the four
threshold *values* within this scheme and found them robust; the
``tmf-pv16-classifier-refine`` topic (``config/research.yaml``) tried finer
discrete subdivisions of the same scheme and all failed G2. No prior file
was found varying the VOL_WIN lookback itself. VOL_WIN=20 spans ~20 minutes
of 1m bars; near the day/night session boundary the trailing 20-bar window
mixes pre-close and post-open volume regimes (thin volume at night), which
is a plausible source of misclassification noise contributing to the
elevated std this audit is trying to reduce.

Original file's docstring follows unmodified for provenance:
...
# Live engine must not put channel_lab on sys.path (architecture cut 2026-08-06).
...
def simulate(
    ...
    n = len(C)
    rvol = rvol_series(V, win=int(p.get("pv_vol_win", VOL_WIN)))
