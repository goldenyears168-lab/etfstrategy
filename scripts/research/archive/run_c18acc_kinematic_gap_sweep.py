#!/usr/bin/env python3
"""C18acc · MV gap W20−W5 exit sweep (from timeline cache).

用法：
  PYTHONPATH=src python3 scripts/research/archive/run_c18acc_kinematic_gap_sweep.py \\
...
from research.backtest.archive.c18acc_kinematic_exit_sweep import (  # noqa: E402
    ...
    render_c18acc_kinematic_gap_sweep_md,
    run_c18acc_kinematic_gap_sweep,
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="C18acc · MV gap W20−W5 sweep")
    parser.add_argument(
        "--from-cache",
        type=Path,
    ...
    out_json = args.out or RESEARCH_RRG / f"{stamp}_c18acc_kinematic_gap_sweep_n{n_slots}.json"
    ...
    out_md.write_text(render_c18acc_kinematic_gap_sweep_md(payload), encoding="utf-8")
