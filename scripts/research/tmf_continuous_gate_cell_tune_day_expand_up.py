#!/usr/bin/env python3
"""Cell retune (2026-08-10): day|expand_up under the NEWLY-LIVE continuous
...
Assigned cell: day|expand_up ONLY. Every other one of the 16 session_pv_book
cells stays at the current-live-equivalent default
(order.tmf_channel_pv16_book.specialized_cell_book()) for the entire run.

IMPORTANT baseline fact (same as the earlier, now-superseded 30m-architecture
attempt at this same cell): in the CURRENT live book, day|expand_up is
already block=["L","S"] (DAY_BLOCKS in freeze_cell_book(), never touched by
CELL_TUNE_V2_PATCHES, only its hang_lo/hang_hi got a patch that is moot
while blocked) -- i.e. it fires ZERO trades today. So "current default"
contributes a fixed net-pnl-delta of 0 on every day by construction, and the
whole question is whether *unblocking* it (with tuned hang/hold params, now
that the underlying regime gate is continuous rather than frozen-at-open)
can extract a positive, day-clustered, OOS-surviving edge.
...
CANDIDATES = [
    ...
    ("unblock_wide_band", dict(block=[], hang_lo=25.0, hang_hi=45.0)),
    ("unblock_wide_band_hold45", dict(
        block=[], hang_lo=30.0, hang_hi=55.0, max_hold_bars=45)),
    ("unblock_narrow_band_hold15", dict(
        block=[], hang_lo=10.0, hang_hi=22.0, max_hold_bars=15)),
...
def main():
    ...
    print(f"baseline day|expand_up trade counts (should be all 0): {baseline_n_vals}")
