...
仍未解的：改善約 <b>2/3 來自科技偏離</b>（科技權重 94.6% vs 宇宙 75.3%）、
1/3 才是同業內選股；對「各檔自己的產業」為基準 t=+1.81（僅邊緣顯著）；
合成分數 OOS 僅涵蓋 2025–2026 兩個科技大年，樣本很薄。
...
def compose(d: str, sig_d: str, bull, bear, stats, row, track, fresh, log, top):
    ...
    txt.append(f"分數 HS = {HS_W_ZP:.0%} 借券佔股本水位 + {1 - HS_W_ZP:.0%} 散戶持股水位"
               ...
               "但改善約 2/3 來自科技偏離、1/3 才是同業內選股；對各檔自己的產業為基準 t=+1.81；"
               "OOS 僅 2025-2026 兩個科技大年。"
               "不要照著交易。")
    return "\n".join(txt), html
