#!/usr/bin/env python3
"""Render 凱基-城中 Copytrade sweep JSON → self-contained HTML report.

Reuses the Yuanta Songjiang HTML renderer (branch_label from JSON).

  PYTHONPATH=src .venv/bin/python scripts/research/render_kgi_chengzhong_copytrade_html.py
  PYTHONPATH=src .venv/bin/python scripts/research/render_kgi_chengzhong_copytrade_html.py \\
      --json reports/research/kgi-chengzhong-copytrade/20260717_kgi_chengzhong_copytrade_sweep.json
"""
...
def _latest_json(dir_path: Path) -> Path | None:
    hits = sorted(
        dir_path.glob("*_kgi_chengzhong_copytrade_sweep.json"),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Render 凱基-城中 copytrade sweep HTML")
    ...
    if not args.no_trades:
        ...
        try:
            ...
            etf_proxy = payload.get("etf_code_proxy") or "KGI_CZ"
