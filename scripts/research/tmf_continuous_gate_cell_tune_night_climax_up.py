#!/usr/bin/env python3
"""Continuous-gate PV16 cell retune: ASSIGNED CELL = night|climax_up.
...
behaves differently, is the CURRENT-LIVE night|climax_up cell's own
...
NOTE: the current-live night|climax_up cell is block=["L","S"] (fully
blocked -- zero trades by construction, always). So the baseline in-sample
"trades" count for this cell is 0 by definition; the interesting question
is purely whether UNBLOCKING it (with some hang/hold setting) produces a
net-positive, day-clustered, statistically supported edge under the new
continuous gate. All candidates below therefore include several unblocked
parameter combos plus the always-available option of leaving it blocked.

Methodology: same day-clustered paired re-simulation rigor as every prior
cell-tune script in this directory. Only night|climax_up is varied; all
other 15 cells stay at order.tmf_channel_pv16_book.specialized_cell_book()'s
...
def main():
    ...
    print("NOTE: current-live cell is block=['L','S'] -> baseline trades = 0 always by construction.")
    ...
    # BASE_CELL is fully blocked (block=["L","S"]); the sweep below is about
    # whether UNBLOCKING with some hang/hold setting can find real edge
    # under the continuous gate. Candidates: several unblocked hang bands
    # (climax_up = strong up-thrust into resistance; night session), a
    # couple of hold-bar variants, and single-side unblocks (long-only /
    # short-only) since a climax-up regime bar may have asymmetric edge.
    candidates = {
        "current_default_blocked": None,
        "unblock_asis_18_32": dict(block=[]),
        "unblock_tight_14_26": dict(hang_lo=14.0, hang_hi=26.0, block=[]),
        "unblock_wide_20_38": dict(hang_lo=20.0, hang_hi=38.0, block=[]),
        "unblock_asis_hold12": dict(max_hold_bars=12, block=[]),
        "unblock_asis_hold30": dict(max_hold_bars=30, block=[]),
        "unblock_longonly": dict(block=["L"]),
        "unblock_shortonly": dict(block=["S"]),
        "unblock_tight_hold12": dict(hang_lo=14.0, hang_hi=26.0, max_hold_bars=12, block=[]),
    }
    ...
    print(f"\nBest in-sample non-default (unblocked) candidate: {best_name} "
          f"mean={best_stats['mean']:.2f} p={best_stats['p']} trades={best_trades}")

    if best_trades < 15:
        print(f"\n*** THIN SAMPLE: best unblocked candidate only generated {best_trades} trades "
              f"across 22 in-sample days. Verdict=INSUFFICIENT_DATA regardless of sign. ***")
        return

    if best_stats["mean"] <= 0:
        print("\nNo unblocked candidate beats the current fully-blocked default in-sample. "
              "Verdict=NO_IMPROVEMENT (current default -- fully blocked -- kept).")
        return
    ...
    print(f"OOS baseline trades: {n_trades_oos_baseline} (blocked, should be 0)")
