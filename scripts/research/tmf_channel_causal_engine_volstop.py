...
# RESEARCH FORK: file lives at scripts/research/, not src/tmf_channel/, so
# _SRC/_ROOT are computed relative to the repo root rather than assuming
# _HERE's parent is `src` (as in the original src/tmf_channel/causal_engine.py).
...
def amp30_causal(H, L, win: int = 30):
    """RESEARCH FORK (stop_pts vol-adaptive experiment, 2026-08-08): linear
    -recency-weighted 30-bar amplitude, ported verbatim from
    reports/research/channel_lab/struct_atr_buffer_amp_scaled_lab.py's
    amp30_causal(). At index t only bars [t-win, t-1] are used (bar t itself
    excluded) so it is safe to gate a bar-t entry decision."""
    n = len(H)
    w = list(range(1, win + 1))
    wsum = float(sum(w))
    out = [None] * n
    for t in range(n):
        a = t - win
        if a < 0:
            continue
        rngs = [H[i] - L[i] for i in range(a, t)]
        out[t] = sum(r * wi for r, wi in zip(rngs, w)) / wsum
    return out


def amp30_pctile_causal(amp30: list, t: int, ref_win: int = 300, warmup: int = 60):
    """Causal percentile rank (0..1) of amp30[t] vs a trailing rolling
    reference window [t-ref_win, t) of past amp30 values. Ported from the
    same lab file's _amp30_pctile(). Returns None during warmup / insuff.
    history (falls back to baseline stop_pts=150 -- unscaled -- by the
    caller)."""
    if t < warmup:
        return None
    cur = amp30[t]
    if cur is None:
        return None
    a = max(0, t - ref_win)
    hist = [x for x in amp30[a:t] if x is not None]
    if len(hist) < 30:
        return None
    hist = sorted(hist)
    import bisect as _bisect

    rank = _bisect.bisect_left(hist, cur)
...
def simulate(
    ...
    # RESEARCH FORK: vol-adaptive stop_pts. Precomputed once per session;
    # AMP30/percentile machinery ported verbatim from
    # struct_atr_buffer_amp_scaled_lab.py (there it scales
    # struct_atr_buffer_mult; here it scales stop_pts instead -- see
    # scripts/research/tmf_channel_causal_engine_volstop.py docstring).
    _STOP_VOL_SCALE = bool(p.get("stop_vol_scale_mode"))
    _AMP30 = amp30_causal(H, L, win=30) if _STOP_VOL_SCALE else None
    _stop_hi_thr = float(p.get("stop_vol_scale_hi_thresh") or 0.66)
    _stop_hi_mult = float(p.get("stop_vol_scale_hi_mult") or 0.6)

    def _entry_stop_pts(t: int) -> float:
        base = float(p.get("stop_pts") or 0.0)
        if not _STOP_VOL_SCALE or _AMP30 is None:
            return base
        pct = amp30_pctile_causal(_AMP30, t, ref_win=300, warmup=60)
        if pct is not None and pct > _stop_hi_thr:
            return base * _stop_hi_mult
        return base
    ...
    def open_lot(t, side, px, how, reg, rv, *, honor_limit: bool = False, hung_limit=None):
        ...
        lots.append(
            dict(
                ...
                stop_pts_eff=_entry_stop_pts(t),
            )
        )
        if was_flat:
            peak_fav = 0.0
    ...
    def _tick_walk_bar(t: int) -> None:
        ...
        stop_pts = float(
            (lots[0].get("stop_pts_eff") if lots else None)
            if (lots and lots[0].get("stop_pts_eff") is not None)
            else (p.get("stop_pts") or 0.0)
        )
    ...
    for t in range(n):
        ...
        if p.get("tick_native") and tick_index is not None:
            ...
            # RESEARCH FORK: use the entry-time vol-scaled stop_pts_eff
            # attached to lot 0 when stop_vol_scale_mode is on; else the
            # plain config stop_pts (byte-identical to upstream).
            stop_pts = float(
                (lots[0].get("stop_pts_eff") if lots else None)
                if (lots and lots[0].get("stop_pts_eff") is not None)
                else (p.get("stop_pts") or 0.0)
