#!/usr/bin/env python3
"""C18acc · 換倉日（as_of）廣度 zone 路由 sweep vs 進場日路由。

用法：
  PYTHONPATH=src python scripts/run_c18acc_breadth_swapday_sweep.py
"""
...
def _what_changes(cfg: ScoreSwapCConfig) -> str:
    ...
    if cfg.breadth_swap_zones:
        parts.append(f"swap_gate∈{cfg.breadth_swap_zones}@{cfg.breadth_swap_zone_date}")
    if cfg.breadth_pool_mode != "always_fresh":
        parts.append(f"pool={cfg.breadth_pool_mode}")
    if cfg.breadth_challenger_pool_mode != "entry_day":
        parts.append(f"challenger={cfg.breadth_challenger_pool_mode}")
...
def run_sweep(
    ...
    *,
    ...
    grid = configs or C18acc_BREADTH_SWAPDAY_SWEEP
    ...
    swapday_variants = {
        ...
        and "swapday" in v["variant_id"]
    }
    entryday_best = max(
        (
            v
            for v in variants
            if v["variant_id"] == "C18acc-breadth-pool-mono-hot"
        ),
        key=lambda x: x.get("mean_excess_pct") or -999,
        default=None,
    )
    swapday_beats_entryday = bool(
        entryday_best
        and any(
            (v.get("delta_vs_champion_pp") or -999)
            > (entryday_best.get("delta_vs_champion_pp") or -999)
            for v in variants
            if v["variant_id"] in swapday_variants
        )
    )
    ...
    return {
        ...
        "date_end": date_end,
        "routing_note": "swap_day=zone(as_of) for challenger pool / swap gate; entry_day=zone(signal_date) for swap gate or both pools",
        "champion": {
        ...
        "recommendation": {
            ...
            "swapday_beats_entryday_routing": swapday_beats_entryday,
            "entryday_pool_mono_hot_delta_pp": entryday_best.get("delta_vs_champion_pp") if entryday_best else None,
...
def _print_table(payload: dict[str, Any]) -> None:
    ...
    if payload.get("extended_summary"):
        print(f"\nExtended {payload['extended_summary']['date_start']}–{payload['extended_summary']['date_end']}:")
        for v in payload["extended_summary"]["variants"]:
            print(
                f"  {v['variant_id']:<32} {v.get('mean_excess_pct', '—'):>7} "
                f"swaps={v.get('swaps_total')} n={v.get('n_periods')}"
            )
    ...
    if rec.get("swapday_beats_entryday_routing"):
        print("Swap-day routing beats entry-day pool mono-hot on full-sample delta.")
    else:
        print("Swap-day routing does NOT beat entry-day pool mono-hot on full-sample delta.")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="C18acc swap-day breadth routing sweep")
    ...
    out = args.out or RESEARCH_RRG / "20260624_c18acc_breadth_swapday_sweep.json"
