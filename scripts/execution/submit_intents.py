#!/usr/bin/env python3
"""讀取策略層 ExecutionIntent JSON → 解析 → 富邦下單 / 查委託。

策略層只需產出 JSON（schema: execution-intent-v1），勿 import execution。

範例：
  # 預覽（不送單）
  .venv-fubon/bin/python scripts/execution/submit_intents.py \\
    reports/execution/intents/example.json --dry-run

  # 實際送單（需明確 --submit）
  .venv-fubon/bin/python scripts/execution/submit_intents.py \\
    reports/execution/intents/my_strategy_20260621.json --submit

  # 查今日委託
  .venv-fubon/bin/python scripts/execution/submit_intents.py --query-orders
"""
...
from execution.fubon_session import check_python_version, connect_fubon  # noqa: E402
from execution.intent import load_intent_batch  # noqa: E402
...
def _reports_dir() -> Path:
    ...
    rel = str(block.get("root") or "reports/execution")
    return ROOT / rel
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Submit strategy ExecutionIntent batch to Fubon")
    parser.add_argument("intent_file", nargs="?", help="execution-intent-v1 JSON 路徑")
    ...
    if args.query_orders:
        try:
            ...
            from execution.fubon_orders import order_results
    ...
    try:
        session = connect_fubon()
        from execution.fubon_orders import place_batch, resolved_orders_preview
