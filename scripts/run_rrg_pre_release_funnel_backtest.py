#!/usr/bin/env python3
"""RRG pre-release funnel lanes · strict + coverage backtest runner."""
...
from research.backtest.rrg_pre_release_funnel_lanes import (  # noqa: E402
    ...
    run_funnel_backtest,
    write_validated_coverage_to_config,
)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="RRG pre-release funnel lane backtest")
    ...
    if args.write_coverage and result.get("coverage_defs"):
        write_validated_coverage_to_config(result["coverage_defs"], args.config)
        print(f"Updated coverage lanes in {args.config}")

    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    stamp = date.today().strftime("%Y%m%d")
    json_path = REPORT_DIR / f"pre_release_funnel_lanes_{stamp}.json"
    md_path = REPORT_DIR / f"pre_release_funnel_lanes_{stamp}.md"

    cfg = yaml.safe_load(args.config.read_text(encoding="utf-8")) or {}
    md = render_funnel_markdown(result, cfg)
