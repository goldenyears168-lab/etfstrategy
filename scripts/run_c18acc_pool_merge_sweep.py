...
用法：
  PYTHONPATH=src python scripts/run_c18acc_pool_merge_sweep.py
"""
...
def _direction_for(cfg: ScoreSwapCConfig) -> str:
    if cfg.variant_id == CHAMPION_SCORE_SWAP_C_VARIANT_ID:
        return "champion"
    if cfg.entry_pool == "fresh" and cfg.swap_pool == "mono_tier2":
        return "D1_entry_fresh_swap_mono"
    if cfg.candidate_pool == "fresh_union_accel":
        return "D2_union_accel"
    return "D3_strengthen_c"
...
def _what_changes(cfg: ScoreSwapCConfig) -> str:
    if cfg.variant_id == CHAMPION_SCORE_SWAP_C_VARIANT_ID:
        return "baseline"
    parts: list[str] = []
    if cfg.entry_pool and cfg.swap_pool:
        parts.append(f"entry={cfg.entry_pool} swap={cfg.swap_pool}")
    elif cfg.candidate_pool == "fresh_union_accel":
        parts.append("pool=fresh∪(mono_tier2∧ā>0)")
    if cfg.candidate_require_positive_accel:
        parts.append("challenger ā>0 filter")
    if cfg.challenger_gate != "none":
        parts.append(f"challenger_gate={cfg.challenger_gate}")
    if cfg.score_margin is not None and cfg.score_margin != 0.05:
        parts.append(f"margin={cfg.score_margin}")
    return " · ".join(parts) or "same pool"
...
def run_sweep(
    ...
    *,
    ...
    grid = configs or C18acc_POOL_MERGE_SWEEP
...
def _print_table(payload: dict[str, Any]) -> None:
    ...
    print(
        f"{'variant_id':<28} {'dir':<6} {'excess':>7} {'swaps':>5} {'n':>4} "
    ...
    for v in sorted(payload["variants"], key=lambda x: -(x.get("mean_excess_pct") or -999)):
        ...
        d = v["direction"][:5]
        print(
            f"{v['variant_id']:<28} {d:<6} {v.get('mean_excess_pct', '—'):>7} "
    ...
    header = f"{'variant_id':<28}" + "".join(f"{y:>8}" for y in years)
    print(header)
    for v in payload["variants"]:
        by_y = {y["year"]: y.get("mean_excess_pct") for y in v.get("by_entry_year", [])}
        row = f"{v['variant_id']:<28}" + "".join(
    ...
    header = f"{'variant_id':<28}" + "".join(f"{z[:6]:>8}" for z in zones)
    print(header)
    for v in payload["variants"]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="C18acc pool merge sweep")
    ...
    out = args.out or RESEARCH_RRG / "20260624_c18acc_pool_merge_sweep.json"
