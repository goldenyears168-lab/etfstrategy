...
| `recycled_n_cycles` | 實際成交輪數 |
| `signal_capture_pct` | 輪數 / 總訊號日（00981A 訊號密，H 愈長捕獲愈低） |
| `alpha_per_locked_day` | 回收 α / 鎖倉總交易日 |
...
| 模型 | H* (optimal hold) | 回收 α | 捕獲率 | 備註 |
|------|--------|--------|--------|------|
| 单池 1 万 | **9** | +11,043 | 11.6% | 同一筆錢輪動 |
| 固定 9 槽 / 9 万 | **20** | +64,561 | 48.0% | H9 可 100% 捕獲、+39,810 |
| 槽 = H | 20 | +111,463 | ~100% | 等同无约束 |

→ **9 萬九槽**：要全接訊號用 **H9**；要極大化總回收 α 可 **H20** 但漏半數訊號。詳見 `reports/research/00981a-copytrade/20260618_00981a_horizon_fixed_capital.md`。
...
在 **H20 基准**上叠加出场触发器；对照 **10 万 rotation**（H20 槽）回收 α。
...
→ **統計上**至少持有 **5 天**才穩定贏台指；**有限資金下**總回收仍以 **H9** 最佳。
...
5. **決策門檻**  
   - Backfill 後若年化共識可執行輪數 **< 10**：僅作監控特徵，不作獨立策略。  
...
| 項目 | 定義 |
|------|------|
| **主要終點（Primary）** | 相對基準的 **訊號日勝台指勝率 Δ（pp）** 與 **累計 α**（`total_alpha_ntd`：每訊號日各 10k、持倉可重疊） |
| **次要終點（Secondary）** | **單池回收 α**（`recycled_total_alpha_ntd`：僅 1 萬輪動）；Leg 層獲利勝率雙比例 z；配對 **α NTD 差** Wilcoxon |
...
| **決策規則** | **僅當 Primary 改善（Δ勝台指>0 且累計 α 升）且配對 p&lt;0.05** 才建議納入實盤；Leg 層顯著但 Primary 不改善 → **拒絕全局採用** |
...
> **契約更新 2026-06-18**：Primary 改為 **累計 α**（`total_alpha_ntd`）；單池回收 α 為 Secondary。  
> **重跑**：20260618 批次 filter 可自 DB / 封存摘要查閱；H1 報告 [`hypothesis`](../reports/research/00981a-copytrade/20260618_00981a_hypothesis_l1h9.md)。

| # | 假設 | 狀態 | 判決 | Primary Δ勝台指 | 累計 α / Leg 檢定 | 報告 |
|---|------|------|------|-----------------|-------------------|------|
| **H1** | 跳過 5–10 腿訊號日 | ✅ 2026-06-18 | **採納** | **+3.32 pp**（66.84%） | 累計 **+39,983**（↑173）· 單池 +9,098（↓）· 5-10 vs 2-4 p=0.12 | [`hypothesis`](../reports/research/00981a-copytrade/20260618_00981a_hypothesis_l1h9.md) |
...
**小結（累計 α Primary）**：原 §10 九項 filter **仍全拒**（勝台指多為負）；**新增採納** H1 `skip_5_10`；#4 `skip_tx<-3%` 為 **探索採納**（累計 α 升但僅 2 個跳過日，實盤意義有限）。#6 `skip_overextended` 累計 α 略升但勝台指仍降 → 不採全局 skip。
...
- **不成立依據**：勝台指從 63.5% → 52.4%（−11.1 pp）；單池 α 少 +7,977；配對檢定 **不顯著**（p=0.81）→ 砍掉加码 **傷害** 績效，與「加码 資訊量低」直覺部分相反（加码 為多數樣本且貢獻 α）。
- **分開統計**：新进 子樣本 n 過小，**不得**單獨下「新进 更優」結論。

#### #3 Overnight Gap Band

- **規則**：gap = (T+1 open − T close) / T close；skip >+3% 或 <-2%；mild 0~+2%。
- **不成立依據**：skip_extreme 與 mild_band 皆 **降低** 勝台指；極端跳空 leg 獲利勝率 **高於** 溫和區間（leg p=0.02，**與「追價風險」假說相反**）。
...
- **不成立依據**：**風險日勝台指高於安全日**（65.96% vs 61.87%）；skip 宏觀風險日 **減少 α**。TX<-3% 僅 2 日 → 統計上無意義。
...
- **雙層結論**：Leg 層跳過過熱 **顯著較差**（p=0.012）→ 過熱 leg 反而賺；訊號日層 skip **−0.7 pp**、配對 p=0.13 → **不採全局 gate**。
- **解讀**：經理人加碼過熱股在跟單框架下仍有 edge（動量延續），與「隔天被悶殺」假說不符。

#### #7 Chip Confirm

- **規則**：`foreign_net_5d>0` 且融資 5 日增幅 <5%。
- **不成立依據**：雙重通過 leg 勝率與未通過 **無差**（p=0.48）；篩選後勝台指 **−8.6 pp**。
...
- **雙層結論**：Leg 通過 63.52% vs 未通過 59.12%（**p=0.061**，10% 邊際、未達 5%）；訊號日 top30 **−1.22 pp**、回收 α +6,013 vs +11,043 → **拒絕**。
- **仅加码·前30%**（新进 全留）：Δ **−1.48 pp**，無改善。

#### 執行層：限價 −1/−2/−3%

- **規則**：`limit=open×(1−k%)`；日 K `low` 觸價成交（Phase 1 粗估）。
- **不成立依據**：勝台指全面下降；成交 leg 為 **弱勢選擇**（成交市價勝率顯著低於未成交，p<0.0001）→ 與 #8 延遲進場結論一致：**動量跟單不宜等便宜**。
...
- **雙層結論**：eligible leg 勝率 **低於** ineligible（59.59% vs 62.97%，p=0.16）；訊號日僅留 eligible **−8.23 pp**、回收 α +2,991 vs +11,043 → **拒絕**。
...
- **不成立依據**：triple 篩選勝台指 **−5.63 pp**、回收 α +7,462 vs +11,043；triple leg 勝率 **低於** 其他（57.0% vs 61.0%，p=0.44）→ **拒絕**。
...
|------|--------------|--------------|------|
| **選股 vs 跟單** | 從 universe **該買誰**（VCP／p6／chunge） | 經理人**已加碼**後 **跟不跟哪幾腿** | 橫截面有效因子，在「已加碼」條件樣本裡辨識力下降或反向 |
...
|------|------------|---------------------------|
| **Primary** | 訊號日 **勝台指 Δ** + 回收 α；採納需配對 **p&lt;0.05** | Leg 層有訊號（#8 p=0.043、#9 p=0.061）但日層不改善 → 仍判 **拒絕** |
| **資金** | 被 skip leg **不重新分配** | 多腿日砍掉「因子判差」腿，常同步砍掉**當日最賺腿**；α 直接消失 |
...
| 9 | 拒絕 | leg 邊際 p=0.061；日層 **−1.2 pp** | top30% 略優但多腿日 skip 低 conviction 傷組合 α |
...
| **P1** | **R-D** | **分層樣本**：新进 vs 加码、单腿 vs 多腿日 分開估 | 避免 96% 加码 稀釋 | 至少一層 Primary 改善 |
...
**事前登錄（R-A/R-B）Primary**：仍用 **回收 α** 與 **勝台指 Δ**；Secondary 報 leg IC。R-A/R-B 若通過，§10.2 原「拒絕」改標 **「skip 不採用；加權／反向可採」**，不刪原報告。
...
**採納門檻（§10.7.4）**：Δ勝台指 &gt; 0 **且** **累計 α** 提升 **且** p(W) &lt; 0.05（單池回收 α 僅 Secondary 參考）。

#### R-A · Leg 加權（通過×1.5／未通過×0.5 · 全日 10k）

| 變體 | Δ勝台指 | 累計 α | 單池 α | p(W) | 解讀 |
|------|---------|--------|--------|------|------|
...
| H2 mom_core | −0.86 pp | **+43,228（↑）** | +11,861（↑） | **0.036** | 累計顯著升但勝台指降 |
...
2. **部分因子改加權後回收 α 上升**（#9/#7/#3 加權）→ 支持 §10.7「動作錯置」假說，但 **未達** 三條件採納門檻。  
3. **#6 非過熱加權** 為 R-A 唯一勝台指正 Δ（+0.86 pp）→ 後續 R-C 可對 `skip_overextended` 做 leg IC。  
...
2. **§10 已證偽的是**：「在 L1H9 等權跟單上，再加一層 **全局 binary skip** 能提升日度勝台指」——此命題目前 **不成立**。  
...
| 出場 | 進場後第 **H** 交易日收盤賣 |
| Primary | 每訊號日 10k、持倉可重疊 → `total_alpha_ntd`、勝台指率 |
...
| 項目 | 定義 |
|------|------|
| **H0** | 5–10 桶延長至 H20 不能同時提升累計 α 與勝台指率（相對 H9） |
...
**採納門檻**：5–10 桶配對日 · cum α(H20) > cum α(H9) **且** 勝台指率升 **且** 配對 Wilcoxon p<0.05。
...
- L1-F1 的 Primary 採納（累計 α）在 **單池 1 槽** 下被捕獲率下降抵消（P2 回收 +4,446 < P1 +11,043）。
- **有足夠槽位時 P2 仍值得做**：9 槽下 P2 回收 α **+38%**（+15,348 vs P1）。
- 落地方式：**5–10 日獨立 H20 槽**，與全局 H9 槽並行，而非單池混用。
- P4 skip 提捕獲率（14.4%）但 1 槽回收 α 仍低於 P1。
