"""OrderIntent · 策略層 → 下單層契約（JSON SSOT）。

策略 / research 腳本只需寫入符合 schema 的 JSON，勿 import order package。
下單層讀取後轉為富邦 Neo Order 並送出。
"""
...
class OrderIntent:
    ...
    - ``target_shares``：目標庫存；下單層依現有庫存計算 delta
    """
...
class OrderIntentBatch:
    ...
    def validate(self) -> None:
        allowed = {SCHEMA_VERSION, *LEGACY_SCHEMA_VERSIONS}
...
def dump_intent_batch(batch: OrderIntentBatch, path: Path | str) -> None:
    payload = {
