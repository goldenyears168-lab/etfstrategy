...
def _submit_one(
    ...
    *,
    ...
    update_intent_submit_status(intent_path, status="submitted", reason=lifecycle.get("lifecycle_status", ""))
...
def process_abc_v3_f1_orders(
    *,
    ...
    for sig in hits:
        ...
        last = results[-1]
        if available_cash is not None and last.get("status") in ("submitted", "reconciled"):
