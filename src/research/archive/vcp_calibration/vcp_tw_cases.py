"""Load config/vcp_tw_cases.yaml for TW VCP gold-standard benchmark."""
...
DEFAULT_CASES_PATH = PROJECT_ROOT / "config" / "vcp_tw_cases.yaml"
BAR_WARMUP_DAYS = 400
...
class VcpTwCasesConfig:
    ...
    def cases_for_ticker(self, ticker: str) -> tuple[VcpTwCase, ...]:
        return tuple(c for c in self.cases if c.ticker == ticker)
    ...
    *,
    ...
    for case in config.cases:
        ...
        for thr in thresholds:
            ...
            qualified = [
                s for s in ticker_signals if float(s["composite_score"]) >= thr
            ]
            in_lit = [
                s
                for s in qualified
