...
from execution.fubon_session import FubonSession
from execution.intent import ExecutionIntentBatch, ResolvedOrder, resolve_intents
...
def _map_enum(cls: Any, name: str, *, field: str) -> Any:
    key = "".join(part[:1].upper() + part[1:] for part in name.split("_"))
    try:
        return getattr(cls, key)
    except AttributeError as exc:
        raise ValueError(f"unsupported {field}: {name}") from exc
